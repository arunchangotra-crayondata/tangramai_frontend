...
interface DisplayISV {
  ...
  processingStatus: "Pending" | "Approved"
  mouFile?: {
    name: string
    url: string
  }
      ...
      <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
        <SheetHeader className="space-y-1">
          <div className="flex items-start justify-between">
            <div>
              <SheetTitle className="text-2xl">ISV Details</SheetTitle>
              <p className="text-sm text-muted-foreground mt-1">Account Information for Individual Software Vendors</p>
            </div>
        ...
        <div className="mt-6 space-y-6">
          ...
          <div>
            ...
            <Input value={isv.name} readOnly className="mt-1.5 bg-gray-50" placeholder="Full name" />
          </div>

          {/* Position */}
          <div>
            <label className="text-sm font-medium">Position</label>
            <Input value={isv.position} readOnly className="mt-1.5 bg-gray-50" placeholder="Position in company" />
          </div>

          {/* Registered Name */}
          <div>
            <label className="text-sm font-medium">Registered Name</label>
            <Input value={isv.registeredName} readOnly className="mt-1.5 bg-gray-50" placeholder="Registered Name" />
          </div>
          ...
          <div>
            ...
            <Input value={isv.domain} readOnly className="mt-1.5 bg-gray-50" placeholder="Domain name" />
          </div>

          {/* Contact Number */}
          <div>
            <label className="text-sm font-medium">Contact Number</label>
            <Input value={isv.contact} readOnly className="mt-1.5 bg-gray-50" placeholder="+91 9876543210" />
          </div>

          {/* MOU File */}
          <div>
            <label className="text-sm font-medium">MOU</label>
            <div className="mt-1.5 flex items-center justify-between rounded-md border bg-gray-50 px-3 py-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm">{isv.mouFile?.name || "mou.pdf"}</span>
              </div>
          ...
          {/* Status - Only show for approved ISVs */}
          {!isPending && (
            <div>
        ...
        <div className="mt-8">
          {isPending ? (
            <div className="flex gap-3">
              ...
              <Button className="flex-1 bg-black hover:bg-black/90" onClick={() => onApprove(isv)}>
                Approve
              </Button>
            </div>
          ) : (
